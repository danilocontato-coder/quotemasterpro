...
BEGIN
  ...
  RAISE NOTICE '  • get_user_role() → usa user_roles (com fallback)';
  RAISE NOTICE '  • has_role_text() → usa user_roles (com fallback)';
  RAISE NOTICE '  • has_any_role() → usa user_roles (com fallback)';
  RAISE NOTICE '';
  RAISE NOTICE '🔄 Sync automático:';
  RAISE NOTICE '  • Trigger criado: profiles.role → user_roles';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️ IMPORTANTE:';
  RAISE NOTICE '  • profiles.role NÃO foi removido (fallback ativo)';
  RAISE NOTICE '  • Monitorar por 1 semana antes de remover';
  RAISE NOTICE '  • Rollback disponível em: docs/rollback_migrations/';
  RAISE NOTICE '';
  RAISE NOTICE '📋 Próximos passos:';
  RAISE NOTICE '  1. Testar login de todos os roles';
  RAISE NOTICE '  2. Verificar permissões em quotes/products';
  RAISE NOTICE '  3. Monitorar logs por 1 semana';
  RAISE NOTICE '  4. Se estável, aplicar migration para remover profiles.role';
  RAISE NOTICE '==========================================';
END;
$$;
