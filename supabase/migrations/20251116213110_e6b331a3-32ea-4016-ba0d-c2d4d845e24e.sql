-- Sprint 1: Create client_invitation_letter_counters table
...
CREATE INDEX idx_invitation_letter_counters_client_year 
ON client_invitation_letter_counters(client_id, year);

-- Sprint 2: Drop old trigger and function
DROP TRIGGER IF EXISTS trg_generate_letter_number ON invitation_letters;
DROP FUNCTION IF EXISTS trigger_generate_letter_number();
...
-- Sprint 4: Migrate existing data - populate counters for clients with existing letters
INSERT INTO client_invitation_letter_counters (client_id, current_counter, year)
SELECT 
  client_id,
  COALESCE(
