-- Corrigir a função approve_offline_payment para lidar corretamente com campos de texto
...
BEGIN
  ...
  IF p_approved THEN
    ...
    -- Create audit log with safe JSON conversion
  ...
  ELSE
    -- Reject payment: change status back to pending and clear offline data
    UPDATE payments
    SET 
      status = 'pending',
      review_notes = COALESCE(p_notes, ''),
      ...
      transaction_id = NULL,
      offline_notes = NULL,
      offline_attachments = ARRAY[]::TEXT[],
      updated_at = NOW()
    WHERE id = p_payment_id;
