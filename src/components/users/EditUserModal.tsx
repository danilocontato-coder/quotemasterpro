...
export function EditUserModal({ open, onClose, user }: EditUserModalProps) {
  const { updateUser, resetPassword, toggleUserStatus, addUserToGroup, removeUserFromGroup } = useUsers();
  ...
  const [formData, setFormData] = useState({
    ...
    status: user?.status || 'active' as UserType['status'],
    client: user?.client || '',
    supplier: user?.supplier || '',
    groupIds: user?.groupIds || [] as string[]
  });
  ...
  const handleToggleStatus = () => {
    ...
    toast.success(`Usuário ${formData.status === 'active' ? 'desativado' : 'ativado'} com sucesso`);
  };
        ...
        <Tabs defaultValue="basic" className="w-full">
          ...
          <TabsContent value="basic" className="space-y-4">
            ...
            <div>
              ...
              <Select 
                value={formData.role} 
                onValueChange={(value) => setFormData(prev => ({ ...prev, role: value as UserType['role'] }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
              ...
              <CardContent className="space-y-4">
                ...
                {user?.loginCredentials && (
                  <div className="p-3 bg-muted rounded-lg">
                    <div className="text-sm font-medium mb-2">Informações de Login:</div>
                    <div className="space-y-1 text-sm">
                      <div><strong>Usuário:</strong> {user.loginCredentials.username}</div>
                      <div><strong>Última alteração:</strong> {new Date(user.loginCredentials.lastPasswordChange).toLocaleDateString()}</div>
