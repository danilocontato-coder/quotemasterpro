...
import { useSupabaseUsers, UserGroup } from '@/hooks/useSupabaseUsers';
...
export function GroupManager({ open, onClose, users }: GroupManagerProps) {
  ...
  const customGroups = groups.filter(g => !g.is_system_group);
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1">
                            <Badge variant="secondary" className="text-xs">
                              {group.user_count} usuário{group.user_count !== 1 ? 's' : ''}
                            </Badge>
                          </div>
                        </div>
                      </CardHeader>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {group.permissions.slice(0, 3).map(permission => (
                            <Badge key={permission} variant="outline" className="text-xs">
                              {permission === 'all' ? 'Todas as permissões' : permission.split('.')[1]}
                            </Badge>
                          ))}
