...
export function EditProductModal({ product, open, onOpenChange }: EditProductModalProps) {
  ...
  useEffect(() => {
    if (product) {
      ...
      setIsCustomCategory(!allCategories.includes(product.category));
  ...
  const handleImageSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!product || !event.target.files) return;
    ...
    for (const file of validFiles) {
      try {
        ...
        toast({
          ...
          description: `${file.name} foi adicionada ao produto.`,
        });
      } catch (error) {
        toast({
          title: "Erro no upload",
          description: `Não foi possível adicionar ${file.name}.`,
          variant: "destructive",
        });
      }
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Produto - {product.code}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="custom-category">Usar categoria personalizada</Label>
            </div>

            {isCustomCategory ? (
              <div className="space-y-2">
                <Label htmlFor="custom-category-input">Categoria Personalizada</Label>
                <Input
